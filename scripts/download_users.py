...
if __name__ == '__main__':
    ...
    parser.add_argument('id_files', nargs='+',
                        help='list of paths to files containing user ids, following special naming format')
    ...
    parser.add_argument('--skipped_log', help='path to txt file containing user ids which were skipped')
    ...
    parser.add_argument('--processed_ids', help='path to txt file containing processed user ids to skip')
    ...
    for filepath in args.id_files:
        ...
        for user_ids in tqdm.tqdm(list(chunked(ids, n=100, strict=False)), desc=f'processing {filepath}'):
            try:
                response = api.lookup_users(user_id=user_ids, tweet_mode='extended')
                # response = api.lookup_statuses(user_ids, tweet_mode='extended', map=True)
            ...
            exceptions = 0
            processed_ids.extend(user_ids)
            users.extend([user._json for user in response])
